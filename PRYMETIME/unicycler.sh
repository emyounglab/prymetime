...
if [[ -s "./cir_rep_contigs.fasta" ]]; then
  ...
  seqkit rename "$PREFIX"_comb.fasta | seqkit seq -m 1000 | seqkit sort --by-length --reverse | awk '/^>/ { if (/circular=true/==1) { print (">scaffold_"++i"_circ") } else { print (">scaffold_"++i) } }' > "$PREFIX"_final.fasta
else
